...
module.exports = flow = function(temporaryFolder) {
  ...
  $.get = function (req, callback) {
    ...
    const params = query.split('&').reduce((prev, curr) => {
      const [k, v] = curr.split('=');
      return {
        ...prev,
        [k]: isNaN(+v) ? v : parseInt(v)
      };
    }, {});
    var chunkNumber = params['flowChunkNumber'] || 0;
    var chunkSize = params['flowChunkSize'] || 0;
    var totalSize = params['flowTotalSize'] || 0;
    var identifier = params['flowIdentifier'] || '';
    var filename = params['flowFilename'] || '';
   ...
   * @param {(status: string, filename?: string, original_filename?: string, identifier?: string) => void} callback
   */
